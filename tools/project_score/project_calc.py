...
# 计算单个软件的用户平均分
def getSingleCommentsAvg(project_id):
    # 查询某一产品的有效评论个数
    sql = "select count(*)  from project_comments  where project_id  = %d and  category = 0 and state = 1 and user_id >0;" % project_id
    sql_result = Mysql('yellowPageDB').select(sql)
    print(sql_result)
    if sql_result[0][0] > 0:  # 有有效评论
        ...
        # 上面评论条数，每一条单独评论的评分计算
        sql_single_comment = "select id,project_id,user_id,score,state from project_comments where   project_id = %d and  category = 0  and state = 1 and user_id >0;" % project_id
        sql_single_comment_result = Mysql('yellowPageDB').select(sql_single_comment)
        total_score = 0
        for i in sql_single_comment_result:
            comment_id = i[0]
            user_id = i[2]
            score = i[3] / 5
            sql_attr = "select project_comments_id,avg(cast(score/10 as decimal(20, 18))) as average_score from project_comments_attr where project_comments_id=%d and score >0;" % comment_id
            sql_attr_result = Mysql('yellowPageDB').select(sql_attr)
            print(sql_attr_result)
            if sql_attr_result[0][0] == None:
                attr_avg = 0
            else:
                attr_avg = sql_attr_result[0][1]
            print(attr_avg)
            single_score = (Decimal(score / 2) + Decimal(attr_avg)) * 50 / 100
            print('每个用户的评分是:{}'.format(single_score))

            # 所有用户的分数之和
            total_score += single_score
        print('所有用户的分数之和是:{}'.format(total_score))
        avg = round(total_score / count_comments, 18)
    else:
        count_comments = sql_result[0][0]
        total_score = 0
        avg = 0
    print('project_id = {} 用户平均分是{}'.format(project_id, avg))

    return count_comments, total_score, avg


# 计算单个软件的用户加权分
def calWeightedScore(project_id):
    all_avg = 3.315406976744186047
    count_comments, total_score, single_avg = getSingleCommentsAvg(project_id)
    single_weighted_score = Decimal((count_comments / (count_comments + 2))) * (Decimal(all_avg) + Decimal(single_avg))
    print('project_id = {} 用户加权分：{}'.format(project_id, round(single_weighted_score, 18)))
    return round(single_weighted_score, 18)


# 计算所有软件的用户加权分
def calWeighted():
    # 获取平均分
    sql_all_avg = "select avg from project_all_avg ;"
    sql_all_avg_result = Mysql('localhost').select(sql_all_avg)
    if sql_all_avg_result:
        all_avg = sql_all_avg_result[0][0]
    # 获取所有的state= 1的产品
    sql = "select id from project where state =1;"
    sql_result = Mysql('yellowPageDB').select(sql)
    for i in sql_result:
        project_id = i[0]
        count_comments, total_score, single_avg = getSingleCommentsAvg(i[0])
        single_weighted_score = Decimal((count_comments / (count_comments + 2))) * (
                Decimal(all_avg) + Decimal(single_avg))
        print('project_id = {} 用户加权分：{}'.format(i[0], round(single_weighted_score, 18)))
        init_score = 0
        all_score = init_score + round(single_weighted_score, 18)
        #         本地数据库插入数据
        # 当前时间
        created_at = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        sql_insert = "INSERT INTO project_user_score " \
                     "(`project_id`, `score_num`, `score_total`, `average`, `user_score`, `init_score`, `all_score`, `created_at`)" \
                     " VALUES(%d,%d,%s,%s,%s,%s,%s,'%s');" % (
                         project_id, count_comments, total_score, single_avg, round(single_weighted_score, 18),
                         init_score,
                         all_score, created_at)
        Mysql('localhost').insert(sql_insert)
...
def InitData():
    all_avg = 4.260416666666666667

    init_zhishu = random.choice(range(20, 50))
    print('随机数是:', init_zhishu)
    init_score = init_zhishu * all_avg / 100
    least = all_avg * 20 / 100
    more = all_avg * 50 / 100
    print('随机值是：{},最小值是:{},最大值是:{}'.format(init_score, least, more))


# 加上初始化的数据
def lastUserScore():
    sql = "select project_id,score from project_user_score_initial ; "
    sql_result = Mysql('yellowPageDB').select(sql)
    for i in sql_result:
        sql_update = "update project_user_score set init_score = '%s' where project_id = %d;" % (i[1], int(i[0]))
        print(sql_update)
        sql_update_result = Mysql('localhost').update(sql_update)
        sql_update_2 = "update project_user_score set all_score = init_score + user_score where project_id = %d;" % i[0]
        print(sql_update_2)

        sql_update_2_result = Mysql('localhost').update(sql_update_2)
...
def MaxAndMinUserScore():
    sql = "select max(all_score) ,min(all_score) from project_user_score  ;"
...
def NormalUserScore():
    ...
    sql = "select project_id,all_score from project_user_score  ;"
    sql_result = Mysql('localhost').select(sql)
    for i in sql_result:
        ...
        sql_insert = "INSERT INTO `project_user_score_final` " \
                     "( `project_id`,`user_score_max`, `user_score_min`,  `all_score`,`final_user_score`, `created_at`)" \
                     " VALUES(%d,%s,%s,%s,%s,'%s'); " % (
